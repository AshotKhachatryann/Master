#!/usr/bin/env python3
"""
Bridge to use a C++ Huffman codec binary from Python.

Provides a Python-friendly interface (compress/decompress on raw bytes)
that delegates to the huffman_codec CLI binary.  The binary is
auto-compiled from huffman_codec.cpp on first use if not already
present.
"""
...
class HuffmanCompressor:
    """Use the C++ Huffman codec CLI for compression/decompression.

    On construction the class locates (or builds) the huffman_codec
    ...
    def __init__(self, base_dir: Path | None = None):
        ...
        self.source_path = self.base_dir / "huffman_codec.cpp"
        self.binary_path = self.base_dir / "huffman_codec"
        self._ensure_binary()
    ...
    def _ensure_binary(self) -> None:
        ...
        if compiler is None:
            raise RuntimeError(
                "g++ compiler not found. Install g++ to build huffman_codec.cpp"
            )
    ...
    def _run_codec(self, mode: str, data: bytes) -> bytes:
        ...
        try:
            ...
            if result.returncode != 0:
                raise RuntimeError(
                    f"Huffman codec failed (mode={mode}): {result.stderr.strip()}"
                )
    ...
    def decompress(self, data: bytes) -> bytes:
        """Decompress a Huffman-coded byte buffer back to the original data."""
